...
Numerical results of the convergence and the stress distribution localized in a section far from the excavation face, obtained from the 3D finite element model, are compared with the analytical solution for the plane strain derived by [\citenum{MA2020}]. The simulations also utilized symmetry concerning the midplane between the twin tunnels. Denoting by $u_y$ the displacement component along the y-axis, Fig.~\ref{MA_convergence_profile_B} displays the convergence curves $U_B = -u_y(B)/R_t$, that characterize the inward movement at the tunnel roof $B(x = 0, y = R_t, z)$ as a function of normalized longitudinal distance to the facing, using the following data:
...
The authors consider symmetry in the midplane between twin tunnels and assume that the plastic zones around each tunnel are isolated from each other and completely surround each tunnel.  Their analytical solution is verified with plane strain numerical solutions obtained using the finite difference program FLAC3D. 

Denoting by $u_y$

As a result, the authors obtained the contour of the plastic zone for several initial conditions and constitutive parameters, i.e., cohesion $c$ and friction angle $\phi$.

Fig.~\ref{MA_FIG1} shows the comparison between the 3D F.E. Solution (from a far behind the excavation face) and the analytical solution for plastic zone boundary provided in [\citenum{MA2020}]. For these analysis, $R_t = 1$ m, $d_1 = 5$ m, Young's modulus $E=20$ GPa, Poisson's ratio $\nu = 0.3$ and, friction angle $\phi = 30^\circ$. The boundary of the plastic zone agrees very well with the elements that have reached the plasticity criterion.

\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.7]{MA_Comparisions_plastic_zones.pdf}
	\caption{Comparisons between the numerical and analytical solution for the plastic zones considering $d_1 = 5$ and $\phi = 30^\circ$ for different initial conditions and cohesion values.}.
	\label{MA_FIG1}
\end{figure}
\FloatBarrier
