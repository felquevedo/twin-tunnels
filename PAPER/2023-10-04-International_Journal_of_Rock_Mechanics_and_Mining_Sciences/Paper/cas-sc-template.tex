...
\section{Comparision with elastic analytical solutions}\label{}


As a first verification of the mesh convergence, numerical solutions in elasticity are made to compare with the results obtained from the analytical and numerical solution of Guo et al [\citenum{GUO2021}]. These authors developed an analytical solution in plane state of deformations for the stress field around unlined twin tunnels with hydrostatic pressure. Fig.~\ref{GUO_FIG0} illustrates the main parameters of the domain. The results are shown at points A and B on the tunnel wall.
\begin{figure}[h!]
	\centering
...
The Fig.~\ref{Convergence Profiles in B} shows the convergence profiles in point B considering several distance between longitudinal tunnel axes $d_1$.

\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.8]{Convergence Profiles in B.pdf}
	\caption{Convergence profiles in point B}
...
\begin{figure}[h!]
	...
	\caption{Relationship between Convergence in B and A}
...
\begin{figure}[h!]
	...
	\caption{Tangencial stress concentration factor in point A}
