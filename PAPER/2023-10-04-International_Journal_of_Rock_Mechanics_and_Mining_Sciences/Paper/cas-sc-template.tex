...
In the context of plane strain conditions, Guo et al. [\citenum{GUO2021}] addressed the configuration of deep twin tunnels excavated in a homogeneous elastic medium in which prevails a hydrostatic initial stress distribution. The authors formulated approximate analytical solutions for the stress distribution establishing far behind the face, which are induced in the rock mass by the excavation of two parallel circular tunnels. The model geometry of the twin circular tunnels as well as the loading associated with initial hydrostatic stress are displayed in Fig.~\ref{GUO_FIG0}.
...
Ma et al. [\citenum{MA2020}] developed an analytical approach to evaluate the stresses and the plastic zone interface around twin circular tunnels. Their solution is based on a plane strain condition, with the stress along the tunnel axis representing the middle stress, i.e., $\sigma_{zz} = (\sigma_v+\sigma_h)/2$. The study considers symmetry in the vertical plane between the tunnels and assumes that the plastic zones around each tunnel are isolated from each other and completely surround the cross-section of each tunnel. For the constitutive model, the authors consider perfectly plastic Mohr-Coulomb criterion with associated plastic flow and verified their solution with numerical calculation in the plane state of deformations obtained using the finite difference program FLAC3D. As a result, the authors obtained the contour of the plastic zone for several initial conditions and constitutive parameters, i.e., cohesion $c$ and friction angle $\phi$. 

Fig.~\ref{MA_FIG1} shows the comparison between the present 3D numerical model solution (taken from a section away from the excavation face influence) and the analytical solution for plastic zone boundary. For these analysis, $R_t = 1$ m, Young's modulus $E=20$ GPa and Poisson's ratio $\nu = 0.3$. The boundary of the plastic zone agrees very well with the elements that have reached the plasticity criterion.
...
Fig.~\ref{MA_FIG2} displays the magnitude of displacements $u$ and stresses $\sigma_{rr}, \sigma_{\theta \theta}$ and $\sigma_{zz}$ at the element level for the case with $c=5$ MPa, $d_1=5$ m, $\sigma_h = \sigma_v = 30$ MPa. We adopted a case without excavating the gallery to assess the quality of the mesh. The smoothness observed in the solution between the elements indicates satisfactory discretization.
