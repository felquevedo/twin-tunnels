...
The results are shown at the crown (point B) and sidewall (point A) of the tunnel considering  $R_t = 4$ m, $E = 500$ MPa, $\nu =$ 0.23, $\sigma_v = \sigma_h = 2.2$ MPa. Fig.~\ref{Convergence Profiles in B} shows the convergence profiles in the crown $U_B = -u_y/R_t$ along the normalized longitudinal tunnel axis $z/R_t$ for various normalized distances $d_1/2R_t$, where $u_y$ is the vertical displacement at the point B. These convergence profiles are calculated with the 3D twin tunnel model in Fig.~\ref{Mesh1}, except for the single tunnel which was calculated using an axisymmetric numerical model.
...
Fig.~\ref{Relationship between convergence in B and A} shows the relationship between the convergences in the crown and sidewall at $z/R_t = -25$, showing the ovalization effect that occurs when the longitudinal tunnels get closer and closer together.
...
Fig.~\ref{Tangencial stress concentration factor in A} shows the analytical and numerical solutions of Guo et al [\citenum{GUO2021}] (in blue and green color) together with the results obtained with the current 3D model (in red color). Ling's solution [\citenum{ling1948}], employed by these researchers to validate their analytical and numerical approaches, is also shown (in black color). Extra analyses with $d_1/2R_t = 4$ and $5$ show that, as the longitudinal tunnels become increasingly spaced apart, the solution converges to that of a single tunnel (in dashed black color).
