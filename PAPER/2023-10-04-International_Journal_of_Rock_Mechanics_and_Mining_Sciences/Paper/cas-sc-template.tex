...
The constitutive parameters of the clay rock mass of the eastern Paris basin (in Aisne, France), as detailed in [\citenum{rousset1988}], [\citenum{giraud1993}] and [\citenum{piepi1995}] and shown in Table~\ref{table2}, are used in the following parametric analysis. These parameters are derived qualitatively from various triaxial compression tests, including creep tests conducted mainly in undrained conditions. The Aisne clay rocks are characterized by low porosity (typically less then 20\%), therefore, hydromechanical coupling has minimal significance. The longterm effects primarily stem from material viscosity, with a low proportion attributable to pore pressure redistribution (hydraulic diffusion). The material exhibits high density (ranging from 2.01 to 2.57 g/cm$^3$) and a low average water content (between 3 to 11\%). Another characteristic is that irreversible deformations are observed in cyclic tests even at very small values of axial deformation (less than 0.3\%). Furthermore, for in situ confinement values (approximaly 450 m deep), the maximum deviation remains practically constant, suggesting a Tresca-type failure criterion. In the creep tests, it was observed that creep deformations are of comparable magnitude to deformations observed during instantaneous tests, and there exists a deviatoric stress threshold beyond which creep phenomena initiate. Moreover, the influence of confining pressure on creep phenomena can be disregarded and comparing both behaviors, instantaneous and delayed, reveals that short-term cohesion exceeds long-term cohesion, with the ratio between these two cohesion values ranging between 1.2 and 2.

Table~\ref{table2} also shows the constitutive parameters used in the lining. Typical values for reinforced concrete are used. For the elastic analyses, Young's modulus at 28 days is used, while for the visous analyses, Young's modulus evolves over time as the lining ages. Each section of the coating has an age that starts counting from the moment it is activated in the analysis. In the constitutive model, the age at which shrinkage begins and the age at which loading begins are given as x and y, respectively.
