...
\section{Comparision with elastic analytical solution}\label{}
...
The results are shown at crown (point B) and sidewall (point B) of the tunnel considering  $R_t = 4$ m, $E = 500$ MPa, $\nu =$ 0.23, $\sigma_v = \sigma_h = 2.2$ MPa. The Fig.~\ref{Convergence Profiles in B} shows the convergence profiles in the crown for various longitudinal tunnel axis distances normalized by the longitudinal tunnel diameter $d_1/2R_t$. It can be seen that the closer the longitudinal tunnels, the greater the convergence at the crown.

\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.8]{Convergence Profiles in B.pdf}
...
\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.8]{Relationship between Convergence in B and A.pdf}
...
Fig.~\ref{Tangencial stress concentration factor in A} shows the analytical and numerical solutions of Guo et al [\citenum{GUO2021}] together with the results obtained with the current 3D model. Ling's solution, employed by these researchers to validate their analytical and numerical approaches, is also shown. Extra analyses with $d_1/2R_t = 4$ and $5$ show that, as the longitudinal tunnels become increasingly spaced apart, the solution converges to that of a single tunnel.

\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.8]{Tangencial stress concentration factor in A.pdf}
	\caption{Tangencial stress concentration factor in the sidewall (point A).}
	\label{Tangencial stress concentration factor in A}
\end{figure}
\FloatBarrier

The Fig.~\ref{GUO_FIG1} shows the comparison of the tangential stress distribution around the tunnel's boundary considering  $R_t = 4$ m, $E = 500$ MPa, $\nu =$ 0.23, $d_1 = 2R_t$, $\sigma_x = \sigma_y  = \sigma_z = 2.2$ MPa.
\begin{figure}[h!]
	\centering
